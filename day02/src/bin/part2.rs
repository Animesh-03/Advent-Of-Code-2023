...
fn solve(input: &str) -> u32 {
    ...
    games
        .iter()
        .map(|game| {
            let (mut red, mut blue, mut green) = (u32::MIN, u32::MIN, u32::MIN);

            game.rounds.iter().for_each(|round| {
                round.iter().for_each(|cube| match cube.color {
                    "red" => red = red.max(cube.amount.into()),
                    "green" => green = green.max(cube.amount.into()),
                    "blue" => blue = blue.max(cube.amount.into()),
                    _ => (),
                });
            });
